...
int n, m; 
vector<arr<ll,2>>adj[nax];
bool vis[nax]; 
ll dists[nax]; 

void dijks(ll s){ // O ( V + E log V )
  priority_queue< arr<ll,2> , vector<arr<ll,2>> , greater<arr<ll,2>>> pq ; 
  pq.push({0,s}); 
  ...
  while(!pq.empty()){
    ...
    for(auto x : adj[cur[1]]){
      if(dists[x[0]] > x[1] + cur[0]){
        dists[x[0]] = cur[0] + x[1];
